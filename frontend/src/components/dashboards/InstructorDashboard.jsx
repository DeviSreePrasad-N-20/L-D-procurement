...
export default function InstructorDashboard() {
  ...
  const [category, setCategory] = useState('TRAINING_MATERIAL');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(`Requested ${quantity}× ${itemName} for your class!`);
      setShowForm(false); setQuantity(30); loadHistory();
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-muted">View upcoming classes, request training materials, devices, and licences for your students.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <Card label="Pending Approval" value={history.filter(r => r.status === 'PENDING_REVIEW').length} />
      ...
      {showForm && (
        <div className="bg-surface border border-primary/30 rounded-xl p-6 shadow-md space-y-5">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold text-ink">Request Materials for Class</h3>
      ...
      <div className="bg-surface border border-border rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border"><h3 className="text-lg font-medium">Upcoming Classes</h3></div>
        <table className="w-full text-left text-sm">
          <thead><tr className="border-b border-border bg-canvas/30">
          ...
          <tbody>
            {upcomingClasses.map(cls => (
              <tr key={cls.id} className="border-b border-border last:border-0">
                ...
                <td className="px-6 py-4"><span className="px-2 py-1 text-xs font-medium rounded bg-primary/10 text-primary">{cls.status}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
